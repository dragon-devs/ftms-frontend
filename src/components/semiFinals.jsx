...
const SemiFinals = ({tournamentId}) => {
    const [matches, setMatches] = useState([]);
    const [error, setError] = useState(null);
    const [loading, setLoading] = useState(true); // Initialize loading to true

    const fetchData = () => {
        const API_URL = `${config.apiEndpoint}/${tournamentId}/semifinals`;
                                    ...
                                    <div>
                                        {uniqueTournamentNames.map((tournamentName) => (
                                            <div key={tournamentName}
                                                ...
                                                <div
                                                    className="bg-emerald-300 px-2 text-[10px] drop-shadow-lg font-bold text-slate-900 rounded-xl">
                                                    SEMI-FINALS
                                                </div>
                                                <ChevronUpIcon
                                                    className={`${
